...
engine = create_engine(
    "sqlite:///../mbuu.db", echo=False, connect_args={"check_same_thread": False}
)
...
class User(Base):
    ...
    sales = relationship(
        "Sale", foreign_keys="Sale.buyer_id", back_populates="buyer", cascade="all, delete-orphan"
    )
...
class Sale(Base):
    ...
    def __repr__(self):
        return (
            f"<Sale id={self.id} car_id={self.car_id} buyer_id={self.buyer_id} price={self.price}>"
        )
